...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlotly({
    ...
    plot1 = ggplot(data = sp500_data(),
      ...
      labs(title = paste0("S&P 500 (", input$time_period[1], "-", input$time_period[2], ")"),
           x = "Date",
           y = y_label) +
      theme_bw() +
  ...
  output$plot2 <- renderPlotly({
    plot2 = ggplot(data = multi_year_data(),
      ...
      labs(title = paste0(input$years, "-Year Returns for S&P 500 (", input$time_period[1], "-", input$time_period[2], ")"),
           x = paste0("Starting Year of ", input$years, "-Year Period"),
           y = "Percent Return") +
      theme_bw() +
      theme(axis.text.x = element_text(angle = 45, hjust=1))
